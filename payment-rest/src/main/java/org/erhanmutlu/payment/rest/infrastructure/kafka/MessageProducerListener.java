...
public class MessageProducerListener implements ProducerListener<String, CreatePaymentRequestMessage> {

    private static final Logger logger = LoggerFactory.getLogger(MessageProducerListener.class);

    @Override
    public void onSuccess(String topic, Integer partition, String key, CreatePaymentRequestMessage message, RecordMetadata recordMetadata) {
        logger.info("success for writing topic: {} for uniqueId: {}", topic, message.getUniqueId());

    }

    @Override
    public void onError(String topic, Integer partition, String key, CreatePaymentRequestMessage message, Exception exception) {
